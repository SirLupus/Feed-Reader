		...
		while ($i >= 0 && ( ($this->posting_limit == 0) || ($this->posting_limit > $j) ))
		{
			...
			$sql = 'SELECT post_subject
					...
					WHERE post_subject = "' . $db->sql_escape($subject) . '"
						AND forum_id = ' . (int) $this->poster_forum_destination_id;
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			$db->sql_freeresult($result);
			...
			if (strnatcasecmp($row['post_subject'], $subject))
			{
				...
				$sql = 'SELECT forum_parents, forum_name FROM ' . FORUMS_TABLE . '
					WHERE forum_id = ' . (int) $this->poster_forum_destination_id;
				$result = $db->sql_query($sql);
				...
				{
					$sql = 'SELECT forum_id, topic_first_post_id, topic_last_post_id, topic_time, topic_posts_approved, topic_posts_unapproved, topic_posts_softdeleted, topic_first_poster_name, topic_first_poster_colour FROM ' . TOPICS_TABLE . '
						WHERE topic_id = ' . (int) $this->poster_topic_destination_id;
					$result = $db->sql_query($sql);
					$post_data = $db->sql_fetchrow($result);
					$db->sql_freeresult($result);
					$message = utf8_normalize_nfc($message);

					if ($post_data['forum_id'] <> $this->poster_forum_destination_id)
					{
						$sql = 'SELECT forum_parents, forum_name FROM ' . FORUMS_TABLE . '
							WHERE forum_id = ' . (int) $post_data['forum_id'];
						$result = $db->sql_query($sql);
						$forum_data = $db->sql_fetchrow($result);
						$db->sql_freeresult($result);
				...
				$data = array(
					...
					'topic_attachment'		=> 0,
					'post_id'				=> $post_id,
					'topic_id'				=> (int) $this->poster_topic_destination_id,
					...
					'no_piu'				=> 0,
					'topic_first_poster_name'	=> $post_data['topic_first_poster_name'],
					'topic_first_poster_colour'	=> $post_data['topic_first_poster_colour'],
				);

				$mode = ((int) $this->poster_topic_destination_id <> 0) ? 'reply' : 'post';
				submit_post($mode, $subject, $user->data['username'], POST_NORMAL, $poll, $data, true, true);
			}
			$i--;
			$j++;
		}
